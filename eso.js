...
Bot.login = function() {
	this.bot.login(process.env.BOT_TOKEN || Files.data.settings.token);
};
...
Bot.checkTag = function(content) {
	const tag = process.env.BOT_TAG || Files.data.settings.tag;
...
Actions.checkConditions = function(msg, cmd) {
	...
	switch(restriction) {
		...
		case 3:
			return !isServer;
		case 4:
			return (process.env.OWNER_ID || Files.data.settings.ownerId) && msg.author.id === (process.env.OWNER_ID || Files.data.settings.ownerId);
		default:
			return true;
	}
};
...
Files.saveData = function(file, callback) {
	...
	this.writers[file].write(this.encrypt(JSON.stringify(data, null, 2)), function() {
		if(callback) {
			callback();
		}
	}.bind(this));
